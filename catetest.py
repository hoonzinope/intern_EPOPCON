...
def returnID_list(n_click, cate_no):
    ...
    ID_list = []
    for idx in goods.index:
        query = """
        select ID
        from MWS_COLT_ITEM
        where ITEM_NUM = '{}' and SITE_NAME = '{}'
        limit 100 """.format(goods[goods.index == idx]['ITEM_NUM'][idx], goods[goods.index == idx]['SITE_NAME'][idx])
        print(query)
    ...
    return [html.Img(id = 'image',src = image)for image in list(image_list['GOODS_IMAGE'])]
    # return dt.DataTable(
